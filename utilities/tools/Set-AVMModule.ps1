...
function Set-AVMModule {
    ...
    if (-not $SkipFileAndFolderSetup -and (($resolvedPath -split '[\\|\/]modules[\\|\/](res|ptn|utl)[\\|\/].+?[\\|\/].+').count -gt 1)) {
    ...
    if ($PSCmdlet.ShouldProcess(('Building & generation of [{0}] modules in path [{1}]' -f $relevantTemplatePaths.Count, $resolvedPath), 'Execute')) {
        try {
            $job = $relevantTemplatePaths | ForEach-Object -ThrottleLimit $ThrottleLimit -AsJob -Parallel {
                $identifierElements = $_ -split '[\/|\\]modules[\/|\\](res|ptn|utl)[\/|\\]'
                $resourceTypeIdentifier = ('modules/{0}/{1}' -f $identifierElements[1], $identifierElements[2]) -replace '\\', '/' # modules/res/<provider>/<resourceType>
