...
.EXAMPLE
Set-ModuleFileAndFolderSetup -FullModuleFolderPath '<repoPath>\modules\res\storage\storage-account\blob-service\container'

Results into:
- Added file [<repoPath>\modules\res\storage\storage-account\main.bicep]
- Added file [<repoPath>\modules\res\storage\storage-account\version.json]
- Added file [<repoPath>\modules\res\storage\storage-account\tests\e2e\defaults\main.test.bicep]
- Added file [<repoPath>\modules\res\storage\storage-account\tests\e2e\waf-aligned\main.test.bicep]
- Added file [<repoPath>\modules\res\storage\storage-account\blob-service\main.bicep]
- Added file [<repoPath>\modules\res\storage\storage-account\blob-service\container\main.bicep]

.EXAMPLE
Set-ModuleFileAndFolderSetup -FullModuleFolderPath '<repoPath>\modules\res\storage\storage-account'
...
function Set-ModuleFileAndFolderSetup {
    ...
    if ([String]::IsNullOrEmpty($CurrentLevelFolderPath)) {
        ...
        $repoRoot, $moduleType, $resourceTypeIdentifier = $FullModuleFolderPath -split '[\/|\\]modules[\/|\\](res|ptn|utl)[\/|\\]' # .*/bicep-registry-modules, res|ptn|utl, <provider>/<resourceType>
        ...
        $CurrentLevelFolderPath = Join-Path $repoRoot 'modules' $moduleType $providerNamespace $resourceType
    }

    # Collect data
    $resourceTypeIdentifier = ($CurrentLevelFolderPath -split '[\/|\\]modules[\/|\\](res|ptn|utl)[\/|\\]')[2] # modules/res/<provider>/<resourceType>
